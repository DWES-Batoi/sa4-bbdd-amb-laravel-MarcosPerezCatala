...
@section('content')
  <div class="container mx-auto px-4">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-blue-800">{{ __('Llistat de Jugadoras') }}</h1>
            ...
            @if(auth()->user()?->role === 'administrador')
              ...
              <form method="POST" action="{{ route('jugadoras.destroy', $jugadora) }}" class="inline">
                ...
                <button class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm" type="submit"
                  onclick="return confirm('{{ __('Segur?') }}')">
                  {{ __('Eliminar') }}
                </button>
              </form>
            @endif
