...
<div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md" style="color: black !important;">
    <h1 class="text-2xl font-bold text-black mb-6">{{ __('Resultat final') }}: {{ $partit->local->nom }} vs {{ $partit->visitant->nom }}</h1>
    ...
    <form action="{{ route('partits.update', $partit) }}" method="POST">
        ...
        <div class="mb-4">
            <label for="gols_local" class="block text-black font-bold mb-2">{{ __('Gols') }} {{ __('Local') }} ({{ $partit->local->nom }}):</label>
        ...
        <div class="mb-6">
            <label for="gols_visitant" class="block text-black font-bold mb-2">{{ __('Gols') }} {{ __('Visitant') }} ({{ $partit->visitant->nom }}):</label>
