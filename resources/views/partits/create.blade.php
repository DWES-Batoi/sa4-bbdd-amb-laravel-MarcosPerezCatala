...
@section('content')
    <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-md" style="color: black !important;">
        ...
        <form action="{{ route('partits.store') }}" method="POST">
            ...
            <div class="mb-4">
                <label for="gols_local" class="block text-black font-bold mb-2">{{ __('Gols') }} {{ __('Local') }}:</label>
                <input type="number" name="gols_local" id="gols_local" value="{{ old('gols_local') }}" min="0"
                    placeholder="Opcional"
                    class="w-full border-gray-300 rounded-md shadow-sm text-black bg-white focus:border-black focus:ring-black">
            </div>

            <div class="mb-6">
                <label for="gols_visitant" class="block text-black font-bold mb-2">{{ __('Gols') }}
                    {{ __('Visitant') }}:</label>
                <input type="number" name="gols_visitant" id="gols_visitant" value="{{ old('gols_visitant') }}" min="0"
                    placeholder="Opcional"
                    class="w-full border-gray-300 rounded-md shadow-sm text-black bg-white focus:border-black focus:ring-black">
            </div>
