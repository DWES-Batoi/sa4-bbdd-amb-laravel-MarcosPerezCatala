...
@section('content')
  <div class="container mx-auto px-4">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-blue-800">{{ __("Llistat d'equips") }}</h1>
      ...
      @foreach ($equips as $equip)
        <article
          ...
          <div class="p-4 flex justify-center bg-gray-50 border-b border-gray-100">
            ...
            @else
              <div
                class="h-32 w-32 flex items-center justify-center text-4xl bg-gray-200 rounded-full border border-gray-300">
                🛡️
              </div>
            @endif
          </div>
          ...
          <div class="p-4 text-center flex-grow">
            <p class="text-gray-700 mb-1"><strong>{{ __('Títols') }}:</strong> {{ $equip->titols }}</p>
            <p class="text-gray-700"><strong>{{ __('Estadi') }}:</strong> {{ $equip->estadi->nom ?? __('Sense estadi') }}
            </p>
          </div>
          ...
          <footer class="bg-gray-50 px-4 py-3 border-t border-gray-100 flex justify-center space-x-2">
            {{-- Veure (Público) --}}
            <a class="text-blue-600 hover:text-blue-800 font-medium px-3 py-1"
              href="{{ route('equips.show', $equip) }}">{{ __('Veure') }}</a>

            {{-- Admin --}}
            @if(auth()->user()?->role === 'administrador')
              <a class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm"
              ...
              <form method="POST" action="{{ route('equips.destroy', $equip) }}" class="inline">
                ...
                <button class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-sm" type="submit"
                  onclick="return confirm('{{ __('Segur?') }}')">{{ __('Eliminar') }}</button>
              </form>
            @endif
          </footer>
        </article>
